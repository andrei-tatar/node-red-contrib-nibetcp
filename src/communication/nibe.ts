...
export class Nibe {
  static createTcp(address: string, registerFilePath: string, logger?: Logger) {
    const nibe = Tcp.create(address, logger?.scope("tcp")).pipe(
      map((x) => new Modbus(x)),
  ...
  readRegister(labelOrAddress: string | number, timeoutMsec: number) {
    return this.registers$.pipe(
      ...
      switchMap((reg) =>
        this.readInternal(reg, timeoutMsec).pipe(retry({ count: 3 }))
      )
    );
  }

  writeRegister(
    labelOrAddress: string | number,
    value: number,
    force: boolean,
    timeoutMsec: number
  ) {
    return this.registers$.pipe(
      ...
      switchMap((reg) =>
        this.writeInteral(reg, value, force, timeoutMsec).pipe(
          retry({ count: 3 })
        )
      ),
      ignoreElements()
  ...
  private writeInteral(
    register: RegisterDefinition,
    value: number,
    force = false,
    timeoutMsec = DEFAULT_TIMEOUT
  ) {
    ...
    return this.modbus.writeRegisters(
      register.address % 10000,
      data,
      timeoutMsec
    );
  }
  ...
  private readInternal(
    register: RegisterDefinition,
    timeoutMsec = DEFAULT_TIMEOUT
  ): Observable<RegisterValue> {
    const registerCount =
      register.size === "s32" || register.size === "u32" ? 2 : 1;
    const readKind =
      register.type === "MODBUS_HOLDING_REGISTER"
        ? "READ_HOLDING"
        : "READ_INPUT";
