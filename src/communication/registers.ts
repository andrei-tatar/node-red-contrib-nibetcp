...
export async function loadAllRegisters(
  registerFile: string
  ...
  const registers = lines
    ...
    .map((l) => {
      const parts = l.split("\t");
      const size = /(?:s|u)(?:8|16|32)/.test(parts[sizeIndex])
        ? parts[sizeIndex]
        : parseInt(parts[sizeIndex], 10) || 5;
      ...
      const regFromFile: RegisterFromFile = {
        label: parts[titleIndex],
        type: registerType,
        address: registerAddress,
        divisionFactor: parseInt(parts[divisionFactor], 10),
        unit: parts[unit],
        size: typeof size === "number" ? SIZE_MAP.get(size) : size,
        minValue:
          parts[minValue] === "-" ? null : parseInt(parts[minValue], 10),
        maxValue:
          parts[maxValue] === "-" ? null : parseInt(parts[maxValue], 10),
        defaultValue:
          parts[defaultValue] === "-"
            ? null
            : parseInt(parts[defaultValue], 10),
      };
      return regFromFile;
    })
    .filter(validate);
